...
17. **装包后立即注入输入可能「全都不生效」**（2026-09-25 F7.8 走查实测）：`install -r` + 启动后，
    `input tap`（点底栏 tab）与 `input swipe` **都没反应**，但 UI 正常渲染（状态栏时钟在走）、
    `pidof` 有进程、`logcat -b crash` 为空、`ui_dump.py` 也抓得到节点 ——
    **极易误判成「我改的代码没生效」**（尤其刚改完手势 / 点击类功能时，会去怀疑代码而不是环境）。
    处置：`am force-stop <pkg>` + `monkey -p <pkg> -c android.intent.category.LAUNCHER 1` 重启 App，
    之后 tap/swipe 立刻恢复。
    **判据：先点一个「绝对该有反应」的目标（如底栏 tab）验证输入通道是否活着，再判定功能是否真坏了。**
18. **视觉类改动必须截图看，语义树证明不了「颜色 / 遮挡」**（同批实测）：F7.8 左滑行首版
    「行没有自身底色 → 底层红色动作区透出来」，语义树 / widget 测试全绿（结构和行为都对），
    只有截图能看出那条红色。反之语义树是「有没有这个节点 / 节点在哪」的真值来源。
    **两者交叉验证，别单靠一种。**
...
22. **`uiautomator dump` 写固定文件名 → 会读到「上一次的界面」**（2026-09-25 F7.14 实测，极具误导性）：
    `dump --compressed /sdcard/_ui.xml` 在 **App 刚启动 / 页面切换的瞬间会失败，且不覆盖旧文件**，
    紧随其后的 `cat /sdcard/_ui.xml` 于是**成功返回上一轮的语义树** → 表现是「新功能没生效 / 弹层没出现」，
    极易误判成代码 bug（本轮就把「首次冷启动弹引导」误判成「没弹」，差点回去改代码）。
    **处置**：① `.workbuddy/ui_dump.py` 已改为 **dump 前先 `rm -f /sdcard/_ui.xml`** —— 失败时退化成
    「文件不存在」→ 触发它自带的 `DUMP FAILED` 报警。**别再改回「不删旧文件」的写法。**
    ② 自己写 dump 流程时一律用**独立文件名**（如 `onb_p2.xml`）+ `pull`，不复用固定名。
    ③ **截图与语义树交叉验证**（视觉类必看截图，见第 18 条）。
    **通用判据：语义树与数据库 / 目录状态互相矛盾时（库里 0 条流水、屏幕上却有旧数据），
    先怀疑落盘缓存，别先怀疑代码。**
